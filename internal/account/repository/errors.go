...
type RepoError = errors.AppError

// ConvertDBError converts database errors to AppError
func ConvertDBError(err error, context string) *RepoError {
	if err == nil {
		return nil
	}
...
func NotFoundError(resourceType, identifier string) *RepoError {
...
func DuplicateError(resourceType, field, value string) *RepoError {
...
func QueryError(err error, query string) *RepoError {
	return errors.DBQueryFailed(err, query)
}

// TransactionError creates a transaction error
func TransactionError(err error, operation string) *RepoError {
