...
func NewGRPCServer(port string, logger *logger.AppLogger, middlewareChecker tenantrepo.PermissionChecker) *GRPCServer {
	...
	srv := grpc.NewServer(
		...
		grpc.ChainUnaryInterceptor(
			...
			middleware.LoggingInterceptor(grpcLogger.WithContext("INTERCEPTOR")),
			interceptor.GRPCValidationInterceptor(v),
			interceptor.DeviceInterceptor(),
			middleware.IamInterceptor(),
			middleware.RequirePermissionInterceptor(grpcLogger.WithContext("INTERCEPTOR"), middlewareChecker),
		),
	)
...
func (s *GRPCServer) Start() error {
	...
	s.logger.Info("Tenant service started successfully", map[string]any{"Port": s.port})
